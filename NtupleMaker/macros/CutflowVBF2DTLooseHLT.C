void CutflowVBF2DTLooseHLT(char* filename) {
  ...
  double doubleHPS20TausFilter = tree->Draw("nEvents", "passhltHpsDoublePFTau20BeforeDeepTau>0 && passVBF2DTOff>0", "goff");
  double doubleDeep20TausFilter = tree->Draw("nEvents", "passhltHpsDoublePFTau20LooseDitauWPDeepTauNoMatch_VBF2DTLoose>0 && passVBF2DTOff>0", "goff");
  double singleDeep45TauL1HLTMatchingFilter = tree->Draw("nEvents", "passhltHpsSinglePFTau45LooseDitauWPDeepTauL1HLTMatched_VBF2DTLoose>0 && passVBF2DTOff>0", "goff");
  double jetKinemFilter = tree->Draw("nEvents", "passhltRealDijetFilter_VBF2DTLoose>0 && passVBF2DTOff>0", "goff");
  double jetIDFilter = tree->Draw("nEvents", "passhltVBFLooseIDPFDummyFilter_VBF2DTLoose>0 && passVBF2DTOff>0", "goff");
  double twoMatchedJetsFilter = tree->Draw("nEvents", "passhltMatchedVBFIsoTauTwoPFJets2CrossCleanedFromDoubleHpsLooseDeepTauIsoPFTauHPS20_VBF2DTLoose>0 && passVBF2DTOff>0", "goff");

  double hltVBFTwoDeepTauDecision = tree->Draw("nEvents", "passVBF2DTLooseHLT>0 && passVBF2DTOff>0","goff");
